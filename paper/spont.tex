...
\begin{figure}[h!]
  ...
  \caption{Emergence of spontaneous activity in a population of neurons. Four representative recordings from the first series on replicate 2540 are shown. All spikes captured during the recording interval are displayed for each age.}
  \label{fig:rasters}
\end{figure}

\begin{figure}[h!]
    \includegraphics{../plots/development_plots.pdf}
	\caption{Firing of neurons over time after plating. The metrics shown are (A) population firing rate, (B) firing rate of individual channels and (C) number of active channels, defined as the number of channels with at least one recorded spike.}
